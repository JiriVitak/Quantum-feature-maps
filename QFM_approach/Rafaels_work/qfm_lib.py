...
# -------- circuito PARAMETRIZADO (mesma topologia, ângulos variam por linha) -----
def build_quench_circuit_param(num_qubits, edges_act, m=1):
    """
    edges_act: lista de arestas (i,j) com J_ij != 0 (índices lógicos).
    Usa:
      - um parâmetro por qubit e por step (termos Y_i)
      - um parâmetro por aresta e por step (mesmo ângulo em YZ e ZY)
    Retorna:
      - o circuito
      - param_order tem o formato [ thy(0,0), thy(0,1), ..., thy(0,n-1),
                                    thy(1,0), ..., thy(m-1,n-1),
    ...
    # ordem implícita dos parâmetros: todos thy depois todos the
...
def build_quantum_features_all(
    ...
    """
    Gera a matriz de features quânticas Xq_all (N x n_q) em UM ÚNICO BATCH.
    """
    N, n = X_q_all.shape

    # reordena features -> qubits lógicos
    X_q_reordered = X_q_all[:, perm]

    # arestas ativas (com Jij != 0), (min(i, j), max(i, j)) é para que sempre tenhamos i < j
    edges_act = sorted(
    ...
    # É necessário dar uma dimensão extra para os observables
    obs_isa_broadcast = [[obs] for obs in obs_isa]  # shape (n_obs, 1)    

    # matriz de parâmetros para TODAS as linhas
    theta_values_all = make_theta_matrix_all(
        X_q_reordered, J_dict, edges_act, tau, m
    )


    # depois de qc_t pronto e theta_values_all pronto, é necessário reordenar as colunas de theta_values_all
    # de forma que fique na ordem dos parâmetros do circuito transpilado qc_t

    param_t = list(qc_t.parameters)          # ordem REAL usada pelo Estimator
    pos = {p: k for k, p in enumerate(param_order)}  # ordem usada na matriz de parâmetros

    idx = [pos[p] for p in param_t]          # como permutar colunas
    ...
    result = job.result()[0]
    evs = np.asarray(result.data.evs, dtype=float)   # deve sair algo tipo (n_obs, N_amostras) 

    # features por linha (N_amostras, n_obs)
    Xq_all = evs.T
...
def build_quantum_features_all_ideal(
    ...
    """
    Gera Xq_all (N x n_obs) em UM ÚNICO BATCH, assumindo simulação ideal (qubits lógicos).

    Premissas:
      - X_q_all: (N, n) já está na ordem dos qubits lógicos (não usa perm).
      - J_dict contém correlações 2-a-2 entre as features; como estamos em simulação ideal,
        tratamos o grafo como completo (todas as arestas (i,j) com i<j).
      - phys_nodes/layout não são usados.
      - Não aplica layout em observables.

    Retorna:
      - Xq_all: matriz (N, n_obs) com os valores esperados (features quânticas)
      - pairs_2q: lista de pares (i,j) correspondente às features 2-qubits (se Z_observables retornar isso)
    """
    N, n = X_q_all.shape

    # Em simulação ideal, assumimos grafo completo (todas as correlações 2-a-2)
    edges_act = sorted({(min(i,j), max(i,j)) for (i,j) in J_dict.keys()})

    # Circuito parametrizado (lógico)
    qc_param, param_order = build_quench_circuit_param(n, edges_act, m=m)

    # Observáveis no espaço lógico (sem apply_layout)
    obs_list, pairs_2q = Z_observables(n)
    obs_broadcast = [[obs] for obs in obs_list]  # shape (n_obs, 1)

    # Matriz de parâmetros (N_amostras x n_params), na ordem "param_order" (do builder)
    theta_values_all = make_theta_matrix_all(
        X_q_all, J_dict, edges_act, tau, m
    )

    # Reordena colunas para bater com a ordem REAL de parâmetros do circuito (qc_param)
    # (Em geral, qc_param.parameters é a ordem efetiva usada pelo Estimator.)
    ...
    # Único job (batch)
    # Obs: alguns backends/estimators aceitam 'backend' aqui, outros não. Mantive opcional.
    if backend is None:
...
def make_theta_matrix_all(X_q_reordered, J_dict, edges_act, tau, m):
    """
    Gera matriz de parâmetros (todas as linhas) na mesma ordem de build_quench_circuit_param:
...
def make_theta_matrix_cluster_using_global_alpha(
    ...
    cluster_nodes_sorted,   # lista de nós lógicos do cluster (no espaço 0..n_b-1)
    J_dict_global,          # J_dict do run global (no espaço 0..n_b-1, arestas físicas ativas)
    old2new,                # mapeia nó lógico global -> índice local 0..k-1
    edges_act_local,        # lista de arestas locais (i,j) em índices 0..k-1
    tau, m,
    ...
    """
    Gera theta_values_all (N, m*k + m*|E_local|), mas usando o MESMO alpha1 do global.
    """
    ...
    # Jij_local vector (na ordem edges_act_local), extraído do J_dict_global
    # Precisamos mapear (i_local, j_local) -> (i_global, j_global)
...
def Z_observables(n, k_max=2):
    ...
    for k in range(2, k_max+1):
        for idx in combinations(range(n), k):
            ...
            if k == 2:
                uniq_edges.append(idx)  # idx já vem ordenado (i<j)
    return obs, uniq_edges

def Z_observables_edges(n, edges_act):
    """
    Observáveis:
      - Z_i (1-local) para todos i
      - Z_i Z_j para cada aresta única (i<j) em edges_act
    """
    obs = []
    # 1-local
    for i in range(n):
    ...
    # 2-local em cima das arestas ativas (únicas)
    uniq_edges = []
    seen = set()
    ...
    return obs, uniq_edges  # uniq_edges = pares 2-local (para nomes)
...
def test_circuit_t(qc_t, backend):
    ...
    print("pares 2q fora do coupling map =", len(bad))
    ...
    print("Qubits envolvidos em 2q =", involved, f"(de {nq})")
    if involved > 0:
        print(
            "Média 2q por qubit (envolvidos) =",
            sum(per_q.values()) / involved,
        )
    print(
        "Média 2q por qubit (todos) =",
        (sum(per_q.values()) / nq) if nq else 0,
    )
    print("Top carregados:", per_q.most_common(10))

# 4) FUNÇÕES DO HAMILTONIANO E CIRCUITO
def ds_curve(t, T):
    s = t / T
